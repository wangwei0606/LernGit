...
namespace UnityEngine.UI.Extensions
{
    ...
    public class RangeSlider : Selectable, IDragHandler, IInitializePotentialDragHandler, ICanvasElement
    {
        ...
        public override void OnPointerDown(PointerEventData eventData)
        {
            ...
            Debug.LogError(this.HighValue+"  "+this.LowValue);
            float tmp=(this.LowValue-this.MinValue)/(this.MaxValue-this.MinValue);
            Debug.LogError(tmp);
            if (m_HighHandleRect != null && RectTransformUtility.RectangleContainsScreenPoint(m_HighHandleRect, eventData.position, eventData.enterEventCamera) && !(this.HighValue==this.MaxValue && tmp>0.8))
            {
